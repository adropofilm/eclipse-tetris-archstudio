...
public class TetrisImp extends JFrame implements ITetrisImp
{
	...
	*/	
	...
	public void begin(){
		...
		_arch.OUT_IClock.setClock(gameSpeed);
		
		_arch.OUT_IBoardPanel.setBoardPanel(this);
		
		_arch.OUT_ISidePanel.setSidePanel(this);
		...
		addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				case KeyEvent.VK_DOWN:
					if(!isPaused && dropCooldown == 0) {
						_arch.OUT_IClock.setCyclesPerSecond(25.0f);
					}
					break;
				...
				case KeyEvent.VK_LEFT:
					if(!isPaused && _arch.OUT_IBoardPanel.isValidAndEmpty(currentType, currentCol - 1, currentRow, currentRotation)) {
						currentCol--;
					}
					break;
				...
				case KeyEvent.VK_P:
					if(!isGameOver && !isNewGame) {
						isPaused = !isPaused;
						_arch.OUT_IClock.setPaused(isPaused);
					}
					break;
			...
			public void keyReleased(KeyEvent e) {
				 ...
				 */
				case KeyEvent.VK_DOWN:
					_arch.OUT_IClock.setCyclesPerSecond(gameSpeed);
					_arch.OUT_IClock.reset();
					break;
				}
	...
	*/
    ...
    public void startGame ()   {
		...
		_arch.OUT_IClock.setPaused(true);
		...
		while(true) {
			 ...
			 */
			if(_arch.OUT_IClock.hasElapsedCycle()) {
				updateGame();
			}
	...
	private void updateGame() {
		 ...
		 */
		if(_arch.OUT_IBoardPanel.isValidAndEmpty(currentType, currentCol, currentRow + 1, currentRotation)) {
			 ...
			 */
			_arch.OUT_IBoardPanel.addPiece(currentType, currentCol, currentRow, currentRotation);
			 ...
			 */
			int cleared = _arch.OUT_IBoardPanel.checkLines();
			if(cleared > 0) {
				score += 50 << cleared;
			}
	...
	private void renderGame() {
		_arch.OUT_IBoardPanel.repaint();
		_arch.OUT_ISidePanel.repaint();
	}
	...
	private void spawnPiece() {
		 ...
		 */
		if(!_arch.OUT_IBoardPanel.isValidAndEmpty(currentType, currentCol, currentRow, currentRotation)) {
	...
	private void rotatePiece(int newRotation) {
		...
		} else if(currentCol + currentType.getDimension() - right >=  _arch.OUT_IBoardPanel.getColCount()) {
				newColumn -= (currentCol + currentType.getDimension() - right) - _arch.OUT_IBoardPanel.getColCount() + 1;
		}
		...
		} else if(currentRow + currentType.getDimension() - bottom >= _arch.OUT_IBoardPanel.getRowCount()) {
			newRow -= (currentRow + currentType.getDimension() - bottom) - _arch.OUT_IBoardPanel.getRowCount() + 1;
		}
		 ...
		 */
		if(_arch.OUT_IBoardPanel.isValidAndEmpty(currentType, newColumn, newRow, newRotation)) {
